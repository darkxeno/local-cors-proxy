  ...
  proxy.use('/' + cleanProxyPartial, function(req, res, next) {
    try {
      ...
      if(req.url && req.url.includes('/pbi-cluster')){
        const finalUrl = 'https://wabi-west-europe-d-primary-redirect.analysis.windows.net' + req.url.toString().replace('/pbi-cluster','');
        console.log(chalk.blue('Request Proxied -> ' + finalUrl));
        req.pipe(request(finalUrl)).pipe(res);
      } else if((req.header('Referer') && ( req.header('Referer').includes('/reportEmbed?groupId=') || 
           req.header('Referer').includes('reportembed.bundle.min.css') ) ) || 
           (req.url && req.url.includes('13.0.10956.175')) ||     
